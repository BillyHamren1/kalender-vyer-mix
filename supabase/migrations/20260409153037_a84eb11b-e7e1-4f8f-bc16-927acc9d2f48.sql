...
-- Add needs_review columns to bookings
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS needs_review BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS needs_review_reason TEXT;

-- Update track_booking_changes to set needs_review on external updates
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.rigdown_end_time IS DISTINCT FROM NEW.rigdown_end_time THEN
      ...
      has_external_changes := true;
    END IF;
    -- Internal fields: do NOT set has_external_changes
    ...
    -- Set needs_review if booking is already assigned to a project and has external changes
    IF has_external_changes AND OLD.assigned_to_project = true THEN
