...
DECLARE
  ...
  has_external_changes BOOLEAN := false;
  should_skip_review BOOLEAN := false;
  resolved_changed_by TEXT;
  is_external_source BOOLEAN := false;
  header_source TEXT := NULL;
  guc_source TEXT := NULL;
  req_headers TEXT;
BEGIN
  ...
  BEGIN
    should_skip_review := (current_setting('app.skip_review', true) = 'true');
  EXCEPTION WHEN OTHERS THEN
    should_skip_review := false;
  END;

  -- Read the PostgREST forwarded request header (set by the caller edge function).
  -- Only 'booking-import' and 'booking-webhook' are treated as external Booking sources.
  BEGIN
    req_headers := current_setting('request.headers', true);
    IF req_headers IS NOT NULL AND req_headers <> '' THEN
      header_source := NULLIF(req_headers::jsonb ->> 'x-lovable-change-source', '');
    END IF;
  EXCEPTION WHEN OTHERS THEN
    header_source := NULL;
  END;
  ...
  BEGIN
    guc_source := NULLIF(current_setting('app.current_user', true), '');
  EXCEPTION WHEN OTHERS THEN
    guc_source := NULL;
  END;

  -- Audit-log identity: prefer explicit header, then GUC, then Postgres role.
  resolved_changed_by := COALESCE(header_source, guc_source, current_user::text);

  -- External classification requires EXPLICIT opt-in.
  -- Bare service_role is NO LONGER external — internal edge functions run as
  -- service_role and must not flag needs_review.
  is_external_source := COALESCE(header_source, guc_source) IN ('booking-import', 'booking-webhook');
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- needs_review-flaggan får ENDAST sättas av externa Booking-källor
    -- (opt-in via request-header 'x-lovable-change-source' eller GUC 'app.current_user'
    -- med värde 'booking-import' / 'booking-webhook'). Interna edge functions
    -- som kör som service_role räknas INTE som externa längre.
    IF has_external_changes AND OLD.assigned_to_project = true AND NOT should_skip_review AND is_external_source THEN
