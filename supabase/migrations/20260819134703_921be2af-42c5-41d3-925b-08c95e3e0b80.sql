...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transport_assignments_planning_status_check') THEN
    ALTER TABLE public.transport_assignments
      ADD CONSTRAINT transport_assignments_planning_status_check
      CHECK (planning_status IN ('preliminary', 'confirmed'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'transport_assignments_transport_type_check') THEN
    ALTER TABLE public.transport_assignments
