-- Uppdatera track_booking_changes: needs_review-flaggan får ENDAST sättas
-- när ändringen kommer från extern källa (service_role = import-bookings/booking-webhook).
-- Interna UI-ändringar (authenticated) loggas fortfarande i booking_changes,
-- men flaggar inte längre "Uppdaterade bokningar"-listan.
...
BEGIN
  ...
  -- Resolve source EARLY so vi kan filtrera needs_review på källa.
  -- 'service_role'   = edge function (booking webhook / import-bookings)
  -- 'booking-import' / 'booking-webhook' = explicit override via app.current_user
  -- 'authenticated'  = Planning UI-mutation (intern)
  resolved_changed_by := COALESCE(
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- Interna UI-ändringar (authenticated) loggas fortfarande i booking_changes
    -- men hamnar INTE i "Uppdaterade bokningar"-listan.
