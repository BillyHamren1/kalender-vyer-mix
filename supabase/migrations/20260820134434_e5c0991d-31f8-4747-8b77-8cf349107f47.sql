-- Canonical warehouse need from Booking (additive; bookings.id is TEXT so no FK on booking refs)
...
BEGIN
  ...
  SELECT * INTO b FROM public.bookings WHERE id = p_booking_id::text;
  ...
  INSERT INTO public.warehouse_project_inbox (
    ...
    b.booking_number, b.client, b.eventdate::date, 'new'
  )
...
BEGIN
  ...
  EXCEPTION WHEN invalid_text_representation THEN
    RETURN NEW;
  END;
  PERFORM public.ensure_warehouse_booking_need(_uuid);
  RETURN NEW;
END;
$$;
