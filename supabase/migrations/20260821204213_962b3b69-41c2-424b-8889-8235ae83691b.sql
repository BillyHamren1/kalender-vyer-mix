...
BEGIN
  ...
  -- Källkontroll: endast externa Booking-ändringar får flagga packning för granskning.
  BEGIN
  ...
  flag_review := has_changes AND is_external_source AND NOT should_skip_review;
  ...
  IF is_consolidated THEN
    ...
    SET start_date = lp_start,
        ...
        updated_at = now(),
        -- Endast påbörjade packningar (frusen snapshot) behöver granskning
        needs_packing_review = CASE WHEN flag_review AND status IS DISTINCT FROM 'planning' THEN true ELSE needs_packing_review END,
        needs_packing_review_reason = CASE WHEN flag_review AND status IS DISTINCT FROM 'planning' THEN 'booking_updated' ELSE needs_packing_review_reason END
