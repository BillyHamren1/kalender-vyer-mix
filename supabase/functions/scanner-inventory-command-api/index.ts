...
import {
  buildBundleScannerInventoryCommand,
  parseScannerInventoryGatewayCommand,
} from '../_shared/scannerInventoryGateway.ts'
...
export async function handleRequest(req: Request): Promise<Response> {
  ...
  const parsed = parseScannerInventoryGatewayCommand(body)
  ...
  const bundleBody = buildBundleScannerInventoryCommand({
  ...
  try {
    upstream = await fetch(`${bundleUrl.replace(/\/$/u, '')}/eventflow-scanner-inventory-command-v1`, {
      method: 'POST',
      headers: {
    ...
    return json(503, { error: 'bundle_inventory_command_unavailable' }, headers)
  }
  ...
  if (upstream.status === 404) {
    return json(503, { error: 'bundle_inventory_command_not_deployed' }, headers)
  }
