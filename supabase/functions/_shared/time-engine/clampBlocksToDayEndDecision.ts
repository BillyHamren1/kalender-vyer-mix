// Time Engine 4.5 — clampBlocksToDayEndDecision
// ──────────────────────────────────────────────
// Pure helper. Tar ett färdigt sätt av ReportCandidateBlock + en redan
// beräknad DayEndDecision, och säkerställer att INGET synligt block kan
// fortsätta efter dayEndDecision.endedAt.
//
...
//   - active timer är öppen efter endedAt (anchor block med isOngoing eller
//     b.endAt > endedAt och b.startAt <= openActiveStartAt):
...
import { stockholmDateKey } from '../stockholmDayWindow.ts';
...
export function clampBlocksToDayEndDecision(
  ...
  let openActiveTimerIgnored = false;
  ...
  for (const raw of blocks) {
    ...
    if (sMs >= endedAtMs) {
      ...
      dropped.push({
        ...
        warningReasons: Array.from(new Set([...(b.warningReasons ?? []), DROP_REASON])),
      });
      // Open active timer som börjar efter endedAt → ignored
      if (openMs != null && Math.abs(sMs - openMs) < 2 * 60_000) {
        openActiveTimerIgnored = true;
      }
      continue;
    }
    ...
    if (eMs > endedAtMs) {
      ...
      // Open active timer-anchor som klipptes vid endedAt
      if (openMs != null && Math.abs(sMs - openMs) < 2 * 60_000) {
        openActiveTimerIgnored = true;
