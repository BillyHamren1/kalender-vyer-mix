...
export const fetchRecentBookingChanges = async (bookingIds: string[]): Promise<BookingChange[]> => {
  ...
  // Triage ska BARA lysa upp ändringar som kommer från Booking-systemet.
  // Sedan 2026-07-20 måste externa importer explicit sätta
  // request-headern 'x-lovable-change-source' (import-bookings → 'booking-import',
  // future booking-webhook → 'booking-webhook'). Rå service_role räknas INTE
  // längre som externt — interna edge functions ska aldrig visas i listan.
  const { data, error } = await supabase
    .from('booking_changes')
    .select('booking_id, change_type, changed_fields, changed_at, changed_by')
    .in('booking_id', bookingIds)
    .in('changed_by', ['booking-import', 'booking-webhook'])
