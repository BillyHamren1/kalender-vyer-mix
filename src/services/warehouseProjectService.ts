...
export class WarehouseProjectAlreadyExistsError extends Error {
  constructor(public existing: WarehouseProject) {
    super(`Lagerprojekt finns redan: ${existing.project_number}`);
    this.name = 'WarehouseProjectAlreadyExistsError';
  }
}
...
export const createWarehouseProjectFromInbox = async (
  ...
  // istället för att försöka skapa duplikat.
  if (inboxItem.source_project_number) {
    ...
    if (existing) {
      // Länka inbox-raden till befintligt projekt och markera som converted.
      await supabase
        .from('warehouse_project_inbox')
        .update({
          status: 'converted',
          processed_at: new Date().toISOString(),
          warehouse_project_id: existing.id,
        })
        .eq('id', inboxItem.id);
      ...
      throw new WarehouseProjectAlreadyExistsError(existing as WarehouseProject);
    }
  }
