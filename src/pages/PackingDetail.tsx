    ...
    <div className="h-full overflow-y-auto" style={{ background: 'var(--gradient-page)' }}>
      <div className="relative">
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-6 max-w-[1600px]">
          ...
          {/* Packing Dates (prominent, always on top) */}
          {booking && !isLargeProject && (
            <div className="mb-4">
              <div className="rounded-2xl border border-border/40 shadow-2xl bg-card p-6">
                <div className="flex items-center gap-2.5 mb-4">
                  <div
                    className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg shadow-warehouse/15"
                    style={{ background: 'linear-gradient(135deg, hsl(38 92% 55%) 0%, hsl(32 95% 40%) 100%)' }}
                  >
                    <Package className="h-5 w-5 text-white" />
                  </div>
                  <span className="text-sm font-bold text-foreground uppercase tracking-wider">Packdatum</span>
                </div>
                <div className="flex items-center gap-3 w-full">
                  {/* Start date */}
                  <Popover>
                    <PopoverTrigger asChild>
                      <div className="flex-1 rounded-xl p-4 border border-border/40 bg-card cursor-pointer hover:border-primary/40 hover:shadow-sm transition-all group">
                        ...
                        {packing.start_date ? (
                          <p className="font-bold text-lg text-foreground tracking-tight">
                            {format(new Date(packing.start_date), 'd MMMM yyyy', { locale: sv })}
                          </p>
                        ) : (
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <div className="flex-1 rounded-xl p-4 border border-border/40 bg-card cursor-pointer hover:border-primary/40 hover:shadow-sm transition-all group">
                        ...
                        {packing.end_date ? (
                          <p className="font-bold text-lg text-foreground tracking-tight">
                            {format(new Date(packing.end_date), 'd MMMM yyyy', { locale: sv })}
                          </p>
                        ) : (
                          <p className="text-sm text-primary font-medium mt-1">Lägg till datum</p>
                        )}
          ...
          {/* Booking Info (rigg/event/nedrivning, address, etc.) */}
          {booking && !isLargeProject && (
            <BookingInfoExpanded
              booking={booking}
