      ...
      <div className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
        <header className="rounded-2xl border bg-card p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div><Button variant="ghost" size="sm" className="-ml-2 mb-2" onClick={() => navigate("/projects")}><ArrowLeft className="mr-2 h-4 w-4" />Alla projekt</Button><h1 className="text-2xl font-semibold">{project.name}</h1><p className="mt-1 text-sm text-muted-foreground">{booking?.booking_number ? `Bokning ${booking.booking_number} · ` : ""}{booking?.client || project.client || "Internt projekt"}</p></div>
            <Button variant="outline" onClick={() => navigate("/calendar")}><CalendarDays className="mr-2 h-4 w-4" />Personal i kalendern</Button>
          </div>
