...
describe("simple project workspace", () => {
  ...
  it("uses the canonical Planning purple for the project header and actions", () => {
    ...
    expect(page).toContain("bg-[image:var(--gradient-icon)]");
    expect(page).toContain("bg-primary-foreground text-primary hover:bg-primary-foreground/90");
    expect(page).toContain("data-[state=active]:bg-[image:var(--gradient-icon)] data-[state=active]:text-primary-foreground");
    ...
    expect(tokens).toContain("--gradient-planner: var(--gradient-icon)");
