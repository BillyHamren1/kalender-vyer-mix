        ...
        {activeKey === "overview" && (
          <div className="space-y-4 mb-6">
            ...
            {bookings.length === 0 ? (
              ...
              // Filter bookings by search query (client, address, booking number)
              ...
              const visibleBookings = q
                ? bookings.filter((lpb) => {
                    ...
                    return (
                      ...
                      (getLargeProjectBookingLabel(lpb) || '').toLowerCase().includes(q)
                    );
                  })
                : bookings;

              const allExpanded = visibleBookings.length > 0 && visibleBookings.every(lpb => expandedBookingIds.has(lpb.booking_id));
              const expandAll = () => setExpandedBookingIds(new Set(visibleBookings.map(lpb => lpb.booking_id)));
              const collapseAll = () => setExpandedBookingIds(new Set());
                ...
                <Card className="border-border/50 shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 border-b border-border/40 bg-muted/20">
                    <div className="relative flex-1 max-w-md">
                      ...
                      <Input
                        placeholder="Sök kund, adress eller bokningsnr…"
                  ...
                  {/* Column header */}
                  <div className="grid grid-cols-12 gap-3 px-4 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground bg-muted/40 border-b border-border/40">
                    <div className="col-span-4 flex items-center gap-2">
                      <span className="w-4" />
                      Kund
                    </div>
                    <div className="col-span-4">Leveransadress</div>
                    <div className="col-span-2">Datum, från</div>
                  ...
                  <div className="divide-y divide-border/40">
                    ...
                    ) : visibleBookings.map((lpb) => {
                      const b = lpb.booking;
                      const isExpanded = expandedBookingIds.has(lpb.booking_id);
                      const isCancelled = (b?.status || '').toUpperCase() === 'CANCELLED';
                      const fromDate = b?.rigdaydate || b?.eventdate || null;
                      const toDate = b?.rigdowndate || b?.eventdate || null;
                      return (
                        <div key={lpb.id}>
                          <div
                            className={cn(
                              "grid grid-cols-12 gap-3 items-center px-4 py-2.5 hover:bg-muted/30 transition-colors cursor-pointer",
                          ...
                          >
                            ...
                            <div className={cn("col-span-4 text-xs text-muted-foreground truncate flex items-center gap-1", isCancelled && "line-through text-muted-foreground/70")}>
                              {b?.deliveryaddress ? (
                                <>
                                  <MapPin className="h-3 w-3 shrink-0" />
                                  <span className="truncate">{b.deliveryaddress}</span>
                                </>
                              ) : <span className="text-muted-foreground/50">—</span>}
                            </div>
                            <div className={cn("col-span-2 text-xs", isCancelled && "line-through text-muted-foreground/70")}>
                              {fromDate ? formatDate(fromDate) : <span className="text-muted-foreground/50">—</span>}
                            </div>
                            <div className="col-span-2 flex items-center justify-between gap-2">
                              <span className={cn("text-xs", isCancelled && "line-through text-muted-foreground/70")}>
                                {toDate ? formatDate(toDate) : <span className="text-muted-foreground/50">—</span>}
                              </span>
                              <Button
                                variant="ghost"
                                size="icon"
