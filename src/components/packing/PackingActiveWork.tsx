...
/**
 * Kompakt "Pågående arbete" — endast faktiskt aktiva packningar.
 * Läser progress via befintlig usePackingProgressBatch (read-only).
 */
const PackingActiveWork: React.FC<Props> = ({ packings }) => {
  ...
  return (
    <section className="rounded-2xl border border-border/40 bg-card shadow-sm overflow-hidden">
      ...
      {active.length === 0 ? (
        <p className="px-5 py-6 text-sm text-muted-foreground">Inget pågående packningsarbete just nu.</p>
      ) : (
