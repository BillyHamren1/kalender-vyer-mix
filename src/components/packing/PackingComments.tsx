...
import { PackingComment } from "@/types/packing";
import { format } from "date-fns";
import { sv } from "date-fns/locale";

interface PackingCommentsProps {
  comments: PackingComment[];
  onAddComment: (data: { author_name: string; content: string }) => void;
}

const PackingComments = ({ comments, onAddComment }: PackingCommentsProps) => {
  const [authorName, setAuthorName] = useState(() => 
    localStorage.getItem('packing_comment_author') || ''
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    localStorage.setItem('packing_comment_author', authorName.trim());
    onAddComment({ 
...
export default PackingComments;
