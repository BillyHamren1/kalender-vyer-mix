...
interface PackingTaskItemProps {
  task: PackingTask;
  onToggle: () => void;
  onClick: () => void;
  commentCount?: number;
...
const PackingTaskItem = ({ task, onToggle, onClick, commentCount = 0 }: PackingTaskItemProps) => {
  ...
  const { data: taskComments } = useQuery({
    queryKey: ['packing-task-comments-count', task.id],
    queryFn: async () => {
...
export default PackingTaskItem;
