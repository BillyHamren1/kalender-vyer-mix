      ...
      <div className="flex items-start justify-between gap-3 p-3 pb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{scheduleGroups.length} personer</p>
          </div>
        </div>
      ...
      <div className="max-h-[560px] space-y-1.5 overflow-y-auto p-2">
        {isLoading || isScheduleLoading ? (
          ...
          scheduleGroups.map(({ staff, items }) => {
            ...
            return (
              <div key={staff.id} className="rounded-lg border border-border/60 bg-background/70 p-2">
                <div className="mb-1 flex items-center justify-between gap-2">
                  <p className="truncate text-xs font-semibold text-foreground">{staff.name}</p>
                  <Badge
                    variant="outline"
                    className={`px-1.5 py-0 text-[10px] leading-tight ${staff.isCurrentlyActive ? 'border-warehouse/40 text-warehouse' : ''}`}
                  >
                ...
                {items.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {dates.map((dateKey) => (
                      <div key={dateKey} className="rounded-md bg-muted/30 p-1.5">
                        <p className="mb-0.5 px-1 text-[11px] font-semibold capitalize text-foreground">
