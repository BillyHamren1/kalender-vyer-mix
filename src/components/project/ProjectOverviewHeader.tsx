...
import { getCurrentProjectPhase, getWorkspaceHealth } from "./projectWorkspaceHealth";
...
const ProjectOverviewHeader = ({
  ...
  const summary = getWorkspaceHealth({
    tasks,
    projectLeader,
    deliveryAddress,
    rigDate,
  ...
  const currentPhase = getCurrentProjectPhase({ rigDate, eventDate, rigDownDate });
  const warnings = summary.attention.map((item) => item.label);
      ...
      <Card className="border-border/60 shadow-sm overflow-hidden">
        <CardContent className="p-0">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <CircleDot className={cn("h-4 w-4", summary.overall === "risk" ? "text-destructive" : summary.overall === "attention" ? "text-amber-600" : "text-emerald-600")} />
                ...
                <p className="text-sm text-muted-foreground">
                  {summary.overall === "risk" ? "Projektet har avvikelser som bör hanteras nu." : warnings.length ? `${warnings.length} punkt${warnings.length === 1 ? "" : "er"} behöver din uppmärksamhet.` : summary.open.length ? "Projektet är under kontroll. Nästa aktivitet visas nedan." : "Inga öppna aktiviteter just nu."}
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-5 divide-x divide-y xl:divide-y-0 divide-border/50">
            ...
            <Metric icon={AlertTriangle} label="Åtgärd" value={warnings.length} sub={summary.overall === "risk" ? "kritiska kontrollpunkter" : warnings.length ? "kontrollpunkter" : "inga avvikelser"} warn={warnings.length > 0} />
          ...
          {warnings.length > 0 && (
            <div className="px-5 py-4 bg-amber-50/70 dark:bg-amber-950/15 border-t border-amber-200/60 dark:border-amber-900/30">
              ...
              <div className="flex flex-wrap gap-2">
                {summary.attention.map((item) => <span key={item.id} className={cn("text-xs rounded-md border bg-background/70 px-2.5 py-1.5", item.level === "risk" ? "border-destructive/30 text-destructive" : "border-amber-200/70")}>{item.label}{item.detail ? <span className="text-muted-foreground"> · {item.detail}</span> : null}</span>)}
              </div>
            </div>
          )}
        </CardContent>
        ...
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
            {phases.map((phase) => {
              ...
              return <div key={phase.key} className={cn("rounded-lg border px-3 py-2.5 transition-colors", active ? "border-primary/40 bg-primary/5 shadow-sm" : "border-border/60 bg-muted/15")}><div className="flex items-center justify-between gap-1.5"><div className="flex items-center gap-1.5"><CheckCircle2 className={cn("h-3.5 w-3.5", value === 100 ? "text-emerald-600" : active ? "text-primary" : "text-muted-foreground")} /><span className="text-xs font-medium">{phase.label}</span></div>{active && <span className="text-[9px] font-semibold uppercase tracking-wide text-primary">Nu</span>}</div><div className="text-[11px] text-muted-foreground mt-1">{value === null ? (active ? "Aktuell fas" : "Ingen aktivitet") : `${value}% klart`}</div></div>;
            })}
          </div>
        </CardContent>
      </Card>
