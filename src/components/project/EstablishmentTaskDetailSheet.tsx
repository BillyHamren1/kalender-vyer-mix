  ...
  const { data: taskDbData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("assigned_to, assigned_to_ids, notes, booking_id, source_product_ids, status, readiness, priority, description, blockers, blocker_responsible, decision_needed, title, start_date, end_date, start_time, end_time, updated_at, linked_entity_type, linked_entity_id, calendar_event_id, visible_in_time_app, visible_in_project_calendar")
        .eq("id", task!.id)
        .single();
      return data;
    },
