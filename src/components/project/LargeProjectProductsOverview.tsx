...
import { Search, Sparkles, ChevronDown, ChevronRight, MoreHorizontal, Trash2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const LargeProjectProductsOverview = ({
  ...
  // Resolve grouped view (only shown if grouping exists)
  ...
  }, [grouping, productById, visibleIds, flatRows]);
  ...
  return (
    <div className="space-y-4 w-full">
      <div className="flex items-center gap-2 w-full">
        ...
        {grouping && (
          <Button variant="ghost" size="icon" onClick={() => clear.mutate()} title="Ta bort gruppering">
                        ...
                        <div className="divide-y divide-border/40">
                          {g.rows.map((row) => (
                            <ProductRow
                              key={row.id}
                              row={row}
                              onMove={() => setMoveDialog({ productId: row.id, name: row.name })}
                            />
                          ))}
                        </div>
                      )}
      ...
      <div className="pt-2 border-t border-border/40 text-xs text-muted-foreground">
        ...
        {grouping && ` · ${grouping.groups.length} kategorier`}
      </div>
