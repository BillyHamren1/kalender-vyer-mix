...
function calculateTaskDates(task: ProjectTask): { startDate: Date; endDate: Date } {
  ...
  // Special handling based on task title
  const lowerTitle = task.title.toLowerCase();
  
  if (lowerTitle.includes('feedback')) {
    // Feedback: starts at deadline (which is 6 days after event), lasts 6 days
    return {
  ...
  if (lowerTitle.includes('stängning')) {
    // Project closing: starts 3 days after event, lasts 7 days (so ends at deadline which is 10 days after event)
    return {
  ...
  if (task.is_info_only) {
    // Info-only tasks (milestones): single day
    return {
      startDate: deadline,
      endDate: deadline
    };
  ...
  // Default: 7 days duration, ending at deadline
  return {
    startDate: subDays(deadline, 7),
    endDate: deadline
  };
...
const ProjectGanttChart = ({ tasks, onTaskClick }: ProjectGanttChartProps) => {
  ...
  const ganttData = useMemo(() => {
    ...
    const ganttTasks: GanttTask[] = tasks
      .filter(t => t.deadline) // Only include tasks with deadlines
      .map(task => {
  ...
  const dayWidth = 40; // px per day
  const rowHeight = 36; // px per task row
  const headerHeight = 60; // px for header
  const taskLabelWidth = 200; // px for task labels
