  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Map<string, string[]>> => {
      ...
      const { data, error } = await supabase
        ...
        .select('warehouse_event_id, staff_id')
      ...
      if (staffIds.length > 0) {
        const { data: staff } = await supabase
          ...
          .select('id, name')
          .in('id', staffIds);
        (staff || []).forEach((s) => names.set(String(s.id), String(s.name)));
      }

      data.forEach((a) => {
        if (!a.warehouse_event_id) return;
        const list = result.get(a.warehouse_event_id) || [];
        const name = a.staff_id ? names.get(String(a.staff_id)) : null;
        if (name && !list.includes(name)) list.push(name);
        result.set(a.warehouse_event_id, list);
      });
      return result;
    },
  });
