 ...
 * ──────────────────────
 * TypeScript-port av SQL-funktionen `compute_workday_review_status`.
 * Tester driver scenarier (missad arrival, stale event, öppen resa, ...)
 * genom oracle och förväntar samma status/reasons som migrations-funktionen.
 *
 * Synkad med UTC day-bound (samma som SQL-migrationen 20260422235220).
 */
...
export function computeReview(input: OracleInput): OracleResult {
  ...
  const staleReview = events.filter(
    (e) =>
      inDay(e.happened_at) && e.resolution_status === 'pending' && e.still_relevant_for_review,
  );
  ...
  const missed = events.filter(
    (e) =>
      inDay(e.happened_at) && e.stale_for_prompt && e.resolution_status === 'pending',
  );
